    ...
    def jmth(func):
        async def wrapper(event):
            ...
            if from_users and event.sender_id not in from_users:
                return await event.answer("هذه ليست لك نصب جـاكثون بنفسك من @xvwwvl", alert=True)
            if owner and event.sender_id not in owner_and_sudos():
                return await event.answer(f"هذا هو بوت جـاكثون الخاص بـ {OWNER} تابعنا @xvwwvl")
            try:
                await func(event)
            except Exception as er:
                LOGS.exception(er)
            ...
            if owner and event.sender_id not in owner_and_sudos():
                IN_BTTS = [
                    [
                        ...
                        Button.url("مجموعة جـاكثون", url="https://t.me/xvwwvl1"),
                    ]
                ]
