...
LOGS.info(f"⏳ تم استغراق {time_formatter((time.time() - start_time) * 1000)} ميللي ثانية لبدء تشغيل سورس جـاكثون.")

LOGS.info(
    """
    ╔══════════════════════════════════════════╗
    ║       ✅ تم تنصيب وتشغيل سورس جـاكثون بنجاح             ║ 
    ║       تابع آخر التحديثات من خلال قناة @xvwwvl            ║
    ╚══════════════════════════════════════════╝
    """
)
...
async def start_bots():
    while True:
        try:
            ...
            LOGS.info("🔄 محاولة تشغيل حساب جـاكثون...")
            await jmubot.start()
