...
async def inline_on():
    ...
    await jmubot.send_message(bot, username)
    await asyncio.sleep(1)
    await jmubot.send_message(bot, "《البوت المساعد من سورس جـاكثون| قناة السورس @xvwwvl | مطور السورس @wwvvwl 》")
    await asyncio.sleep(1)
    ...
    if not log_chat:
        ...
        try:
            group_id = await jmubot(
                CreateChannelRequest(
                    title="مجموعة السجل جـاكثون ",
                    about="مجموعة حفظ الأحداث والتنبيهات الخاصة بسورس جـاكثون\n\n انضم لقناة السورس @xvwwvl",
                    megagroup=True,
                ),
            )
